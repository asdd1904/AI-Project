...
def h2o_pred_proba_bad(model, hf) -> np.ndarray:
...
for _, va_idx in cv.split(X_tr, y_tr):
...
pbad_test_raw = h2o_pred_proba_bad(gbm_final, te_h)
...
with mlflow.start_run(run_name="classification_h2o_xgb"):
    mlflow.log_params({
        "framework": "h2o", "model": "GBM",
        **gbm_params, "calibration": "Platt(LogReg OOF)",
        "threshold_policy": "median(fold_t*)",
        "thr_range": f"[{THRESHOLDS.min():.2f},{THRESHOLDS.max():.2f}]",
        "seed": SEED,
    })
    mlflow.log_metrics({
        "cv_cost_mean": cv_mean, "cv_cost_std": cv_std,
        "thr_global": thr_global,
        "test_cost": test_cost, "test_accuracy": test_acc,
    })

print(f"H2O GBM (cal) — CV cost={cv_mean:.2f}±{cv_std:.2f} | "
